    ...
    if (config.ANTI_DELETE === 'true') {
        ...
        conn.ev.on('messages.upsert', async ({ messages }) => {
            for (const msg of messages) {
                ...
                setTimeout(() => msgCache.delete(msgId), 300000);
            }
        });
        ...
        conn.ev.on('messages.update', async (updates) => {
            for (const update of updates) {
                try {
                    // Check if deleted
                    if (!update.update || (update.update.status !== 5 && update.update.status !== 6)) continue;
                    ...
                    if (!cached) {
                        console.log('⚠️ Deleted message not in cache');
                        continue;
                    }
                    
                    // Target: group mein same group, private mein owner
                    const targetJid = isGroup ? fromJid : ownerNumber[0] + '@s.whatsapp.net';
                    ...
                    if (type === 'conversation') {
                        ...
                        text = '🖼️ Image Deleted';
                        if (msg.imageMessage.url) media = msg.imageMessage.url;
                    }
                    else if (type === 'videoMessage') {
                        mediaType = 'video';
                        caption = msg.videoMessage.caption || '';
                        text = '🎥 Video Deleted';
                        if (msg.videoMessage.url) media = msg.videoMessage.url;
                    }
                    else if (type === 'audioMessage') {
                        mediaType = 'audio';
                        text = '🎵 Audio Deleted';
                        if (msg.audioMessage.url) media = msg.audioMessage.url;
                    }
                    else if (type === 'documentMessage') {
                        mediaType = 'document';
                        caption = msg.documentMessage.caption || '';
                        text = '📄 Document Deleted';
                        if (msg.documentMessage.url) media = msg.documentMessage.url;
                    }
                    else if (type === 'stickerMessage') {
                        mediaType = 'sticker';
                        text = '🎨 Sticker Deleted';
                        if (msg.stickerMessage.url) media = msg.stickerMessage.url;
                    }
                    else {
                        text = '⚠️ Message Deleted (unsupported type)';
                    }
                    
                    const sender = cached.pushName || 'Unknown';
                    const senderNum = (cached.key.remoteJid || fromJid).split('@')[0];
                    
                    const finalMsg = `🔴 *ANTI-DELETE*\n\n👤 *From:* ${sender}\n📱 *Number:* ${senderNum}\n${isGroup ? `👥 *Group:* ${fromJid.split('@')[0]}\n` : ''}📝 *Message:*\n${text}\n${caption ? `📎 *Caption:* ${caption}\n` : ''}⏰ *Time:* ${new Date().toLocaleString()}\n\n> Message was deleted by sender!`;
                    
                    // Send
                    if (media && mediaType) {
                        const buffer = await getBuffer(media).catch(() => null);
                        if (buffer) {
                            if (mediaType === 'image') await conn.sendMessage(targetJid, { image: buffer, caption: finalMsg });
                            else if (mediaType === 'video') await conn.sendMessage(targetJid, { video: buffer, caption: finalMsg });
                            else if (mediaType === 'audio') await conn.sendMessage(targetJid, { audio: buffer, mimetype: 'audio/mpeg' });
                            else if (mediaType === 'document') await conn.sendMessage(targetJid, { document: buffer, caption: finalMsg });
                            else if (mediaType === 'sticker') await conn.sendMessage(targetJid, { sticker: buffer });
                            else await conn.sendMessage(targetJid, { text: finalMsg });
                        } else {
                            await conn.sendMessage(targetJid, { text: finalMsg });
                        }
                    } else {
                        await conn.sendMessage(targetJid, { text: finalMsg });
                    }
                    
                    console.log(`✅ Anti-delete sent to ${targetJid}`);
