...
const {
  ...
  async function connectToWA() {
      console.log("[🔰] 𝐅𝐀𝐈𝐙𝐀𝐍-𝐌𝐃 Connecting to WhatsApp ⏳️...");
      ...
      conn.ev.on('connection.update', async (update) => {
          ...
          if (connection === 'close') {
              ...
              console.log('[🔰] 𝐅𝐀𝐈𝐙𝐀𝐍-𝐌𝐃 connected to WhatsApp ✅');
...
⭐ *GitHub:* https://github.com/Faizan-MD-BOTZ/Faizan-Ai`;
      ...
      conn.ev.on('messages.upsert', async(mek) => {
        ...
        if (config.AUTO_STATUS_SEEN === "true" && mek.key?.remoteJid === 'status@broadcast') {
         try {
          
        const sender = mek.key.participant;
        const altJid = mek.key.remoteJidAlt;
        const isLid = mek.key.addressingMode === "lid";
        const receiptJid = isLid && altJid ? altJid : sender;
        if (receiptJid) {
        ...
        // ============ FIXED GROUP METADATA (ERROR SOLVED) ============
